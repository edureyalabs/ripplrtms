...
export async function getTaskTimeline(
  ...
  const [{ data: events }, { data: updates }] = await Promise.all([
    ...
    supabase
      .from("task_updates")
      .select(
        "id, kind, body, created_at, author:profiles!task_updates_author_id_fkey(full_name, email), task_update_attachments(id, file_name, file_path, mime_type)"
      )
      .eq("task_id", taskId)
      .order("created_at"),
  ]);
  ...
  const updateEntries: TimelineEntry[] = await Promise.all(
    (updates ?? []).map(async (u) => {
      ...
      return {
        id: u.id,
        kind: u.kind as TimelineEntry["kind"],
